...
{
    [TaskDescription("Check to see if any objects are within sight of the agent.")]
    ...
    public class CanSeeObject : Conditional
    {
        ...
        [SerializeField]
        private int m_MaxCollisionCount = 200;
        [Tooltip("The LayerMask of the objects to ignore when performing the line of sight check.")]
        [SerializeField]
        private LayerMask m_IgnoreLayerMask;
        [Tooltip("The field of view angle of the agent (in degrees).")]
        [SerializeField]
        private SharedFloat m_FieldOfViewAngle = 90;
        [Tooltip("The distance that the agent can see.")]
        [SerializeField]
        private SharedFloat m_ViewDistance = 1000;
        [Tooltip("The raycast offset relative to the pivot position.")]
        [SerializeField]
        private SharedVector3 m_Offset;
        [Tooltip("The target raycast offset relative to the pivot position.")]
        [SerializeField]
        private SharedVector3 m_TargetOffset;
        [Tooltip("The offset to apply to 2D angles.")]
        [SerializeField]
        private SharedFloat m_AngleOffset2D;
        [Tooltip("Should the target bone be used?")]
        [SerializeField]
        private SharedBool m_UseTargetBone;
        [Tooltip("The target's bone if the target is a humanoid.")]
        [SerializeField]
        private SharedHumanBodyBones m_TargetBone;
        [Tooltip("Should a debug look ray be drawn to the scene view?")]
        [SerializeField]
        private SharedBool m_DrawDebugRay;
        [Tooltip("Should the agent's layer be disabled before the Can See Object check is executed?")]
        [SerializeField]
        private SharedBool m_DisableAgentColliderLayer;
        [Tooltip("The object that is within sight.")]
        [SerializeField]
        private SharedGameObject m_ReturnedObject;
        [Tooltip("Shared bool indicating whether the player is in sight or not.")]
        [SerializeField]
        private SharedBool canSeePlayer;
        [Tooltip("The last seen location of the player.")]
        [SerializeField]
        private SharedVector3 lastSeenLocation;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            // The collider layers on the agent can be set to ignore raycast to prevent them from interfering with the raycast checks.
            ...
            // Set the shared bool based on visibility
            canSeePlayer.Value = m_ReturnedObject.Value != null;
            if (canSeePlayer.Value == true)
            {

                lastSeenLocation.SetValue(m_ReturnedObject.Value.transform.position);
